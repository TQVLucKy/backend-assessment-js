// .wrangler/tmp/bundle-P2tyB7/checked-fetch.js
...
async function insertProducts(client, product) {
  const { id, title, created_at, updated_at, tags, variants } = product;
  let count = 0;
...
router.get("/api/products", async (req, env) => {
  const client = new _n({ connectionString: env.HYPERDRIVE.connectionString });
...
router.get("/api/products/:product_id", async (req, env) => {
  console.log("req", req);
  const client = new _n({ connectionString: env.HYPERDRIVE.connectionString });
  await client.connect();
  try {
...
router.put("/api/products", async (req, env) => {
  console.log("req", req);
  const client = new _n({ connectionString: env.HYPERDRIVE.connectionString });
  await client.connect();
  try {
...
// .wrangler/tmp/bundle-P2tyB7/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-P2tyB7/middleware-loader.entry.ts
